...
namespace Calculator
{
    ...
    public partial class MainWindow : Window
    {
        public MainWindow()
        {
            ...
            Type type = assembly.GetType("BinCalc.BCalc");

            _BinCalc = Activator.CreateInstance(type);
            _BinCalcDoOperation = type.GetMethods()[0];

            Calc.AddOperation("|", BinCalcAdapterOr);
            Calc.AddOperation("&", BinCalcAdapterAnd);
            Calc.AddOperation("^", BinCalcAdapterXor);
        }

        private readonly object _BinCalc;
        private readonly MethodInfo _BinCalcDoOperation;
        ...
        private double BinCalcAdapterOr(double x, double y)
        {
            return DoBinCalcOperation(x, y, "|");
        }

        private double BinCalcAdapterXor(double x, double y)
        {
            return DoBinCalcOperation(x, y, "^");

        }
        private double BinCalcAdapterAnd(double x, double y)
        {
            return DoBinCalcOperation(x, y, "&");
        }

        private double DoBinCalcOperation(double x, double y, string operation)
        {
            object[] expression = new object[] { x.ToString() + operation + y.ToString() };
            return double.Parse((string)_BinCalcDoOperation.Invoke(_BinCalc, expression));
        }
        ...
        protected void Button_ClickNumber(object sender, RoutedEventArgs e)
        {
            ...
            StatusLabel.Content = "Вставлена цифра " + (string)current.Content;
